...
function matchesForCustomDirective(attributeName) {
    return attributeName.match(/^pb:/);
}

function extractDirective(el, name) {
    let [value, ...modifiers] = name.replace(/^pb:/, '').split('.');
    if (!value) return;
...
class Component {
    ...
    handleModelChange(key, value) {
        ...
        rootElement.querySelectorAll(`[pb:model="${key}"]`).forEach(el => {
            if (el.value !== value) {
                el.value = value;
            }
        });
        ...
        rootElement.querySelectorAll(`[pb:text="${key}"]`).forEach(el => {
            el.textContent = value;
        });

        // Update conditional elements
        rootElement.querySelectorAll(`[pb:if="${key}"]`).forEach(el => {
        ...
        if (currentComponent) {
            morphdom(currentComponent, tempDiv.firstElementChild, {
                onBeforeElUpdated: (fromEl, toEl) => {
                    ...
                    if (fromEl.hasAttribute('pb:model')) {
                        const modelName = fromEl.getAttribute('pb:model');
    ...
    updateDOM(newData) {
        // Mettre à jour tous les éléments avec pb:text
        document.querySelectorAll('[pb:text]').forEach(el => {
            const key = el.getAttribute('pb:text');
            if (newData.hasOwnProperty(key)) {
                el.textContent = newData[key];
            }
        });

        // Mettre à jour tous les éléments avec pb:model
        document.querySelectorAll('[pb:model]').forEach(el => {
            const key = el.getAttribute('pb:model');
            if (newData.hasOwnProperty(key)) {
                el.value = newData[key];
            }
        });

        // Mettre à jour les éléments avec pb:show
        document.querySelectorAll('[pb:show]').forEach(el => {
            const key = el.getAttribute('pb:show');
            if (newData.hasOwnProperty(key)) {
...
class Directive {
    ...
    get params() {
        ...
        if (this.el.tagName.toLowerCase() === 'form') {
            const params = {};
            const modelDirectives = this.el.querySelectorAll('[pb:model], [pb:upload]');

            modelDirectives.forEach(directiveEl => {
                const directiveName = directiveEl.getAttribute('pb:model') || directiveEl.getAttribute('pb:upload');
...
function getFormModels(form, component) {
    const models = {};
    form.querySelectorAll('[pb:model]').forEach(el => {
        const modelName = el.getAttribute('pb:model');
...
// Directive pb:click pour gérer les clics
...
// Directive pb:submit pour gérer les soumissions de formulaire
...
// Directive pb:change pour gérer les changements de valeur
                ...
                if (currentComponent) {
                    morphdom(currentComponent, newContent, {
                        onBeforeElUpdated: (fromEl, toEl) => {
                            ...
                            if (fromEl.hasAttribute && Array.from(fromEl.attributes).some(attr => attr.name.startsWith('pb:'))) {
